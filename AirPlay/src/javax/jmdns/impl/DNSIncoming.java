...
public final class DNSIncoming extends DNSMessage {
    ...
    public DNSIncoming(DatagramPacket packet) throws IOException {
        ...
        try {
            ...
            Log.w("jmdns", "DNSIncoming() dump " + print(true) + "\n exception ", e);
    ...
    private DNSQuestion readQuestion() {
        ...
        if (type == DNSRecordType.TYPE_IGNORE) {
            Log.e("jmdns", "Could not find record type: " + this.print(true));
        }
    ...
    private DNSRecord readAnswer(InetAddress source) {
        ...
        DNSRecordType type = DNSRecordType.typeForIndex(_messageInputStream.readUnsignedShort());
        if (type == DNSRecordType.TYPE_IGNORE) {
            Log.e("jmdns", "Could not find record type. domain: " + domain + "\n" + this.print(true));
        }
        int recordClassIndex = _messageInputStream.readUnsignedShort();
        ...
        if ((recordClass == DNSRecordClass.CLASS_UNKNOWN) && (type != DNSRecordType.TYPE_OPT)) {
            Log.e("jmdns", "Could not find record class. domain: " + domain + " type: " + type + "\n" + this.print(true));
        }
        boolean unique = recordClass.isUnique(recordClassIndex);
        ...
        switch (type) {
            ...
            case TYPE_PTR:
                ...
                if (service.length() > 0) {
                    ...
                    Log.w("jmdns", "There was a problem reading the service name of the answer for domain:" + domain);
                }
                break;
            case TYPE_TXT:
                ...
                if (version == 0) {
                    ...
                    while (_messageInputStream.available() > 0) {
                        ...
                        if (_messageInputStream.available() >= 2) {
                            ...
                            Log.w("jmdns", "There was a problem reading the OPT record. Ignoring.");
                            break;
                        }
                        int optionLength = 0;
                        if (_messageInputStream.available() >= 2) {
                        ...
                        switch (optionCode) {
                            case Owner:
                                ...
                                try {
                                    ...
                                    Log.w("jmdns", "Malformed OPT answer. Option code: Owner data: " + this._hexString(optiondata));
                                }
                                if (Log.isLoggable("jmdns", Log.VERBOSE)) {
                                    Log.d("jmdns", "Unhandled Owner OPT version: " + ownerVersion + " sequence: " + ownerSequence + " MAC address: " + this._hexString(ownerPrimaryMacAddress)
                            ...
                            case UL:
                                if (Log.isLoggable("jmdns", Log.VERBOSE)) {
                                    Log.d("jmdns", "There was an OPT answer. Option code: " + optionCode + " data: " + this._hexString(optiondata));
                                }
                                break;
                            case Unknown:
                                Log.w("jmdns", "There was an OPT answer. Not currently handled. Option code: " + optionCodeInt + " data: " + this._hexString(optiondata));
                                break;
                            default:
                    ...
                    Log.w("jmdns", "There was an OPT answer. Wrong version number: " + version + " result code: " + extendedResultCode);
                }
                break;
            default:
                if (Log.isLoggable("jmdns", Log.VERBOSE)) {
                    Log.d("jmdns", "DNSIncoming() unknown type:" + type);
                }
