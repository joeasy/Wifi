    ...
    public static HostInfo newHostInfo(InetAddress address, JmDNSImpl dns, String jmdnsName) {
        ...
        try {
            if (addr == null) {
                ...
                if (addr.isLoopbackAddress()) {
                    Log.w("jmdns", "Could not find any address beside the loopback.");
                }
            } else {
                aName = addr.getHostName();
            }
            ...
            Log.w("jmdns", "Could not intialize the host network interface on " + address + "because of an error: " + e.getMessage(), e);
    ...
    private HostInfo(final InetAddress address, final String name, final JmDNSImpl dns) {
        ...
        if (address != null) {
            try {
                ...
                Log.w("jmdns", "LocalHostInfo() exception ", exception);
            }
        }
    }
