    ...
    public void start(Timer timer) {
        ...
        for (DNSQuestion question : _in.getQuestions()) {
            if (Log.isLoggable("jmdns", Log.VERBOSE)) {
                Log.d("jmdns", this.getName() + "start() question=" + question);
            }
        ...
        if (Log.isLoggable("jmdns", Log.VERBOSE)) {
            Log.d("jmdns", this.getName() + "start() Responder chosen delay=" + delay);
        }
    ...
    public void run() {
        ...
        if (this.getDns().isAnnounced()) {
            try {
                ...
                for (DNSQuestion question : _in.getQuestions()) {
                    if (Log.isLoggable("jmdns", Log.VERBOSE)) {
                        Log.d("jmdns", this.getName() + "run() JmDNS responding to: " + question);
                    }
                ...
                for (DNSRecord knownAnswer : _in.getAnswers()) {
                    if (knownAnswer.isStale(now)) {
                        ...
                        if (Log.isLoggable("jmdns", Log.VERBOSE)) {
                            Log.d("jmdns", this.getName() + "JmDNS Responder Known Answer Removed");
                        }
                    }
                }
                ...
                if (!answers.isEmpty()) {
                    if (Log.isLoggable("jmdns", Log.VERBOSE)) {
                        Log.d("jmdns", this.getName() + "run() JmDNS responding");
                    }
                ...
                Log.w("jmdns", this.getName() + "run() exception ", e);
                this.getDns().close();
            }
        }
    }
