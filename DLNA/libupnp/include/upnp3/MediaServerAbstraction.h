...
/*!	\brief Allows applications to deallocate the \ref MSA_CdsQuery object
	that was executed by an \ref MSA_Callback_OnBrowse.
...
/*!	\brief Application-level code can call this method within their
	handler of an \ref MSA_Callback_OnBrowse callback
	when they need to respond to the control point with an error.

	\param[in] msa_obj \ref MSA instance. Obtained from \ref MSA_CreateMediaServer().
	\param[in] upnp_session The UPnP session token specified in the \a MSA_Callback_OnXXX callback.
	\param[in] error_code This value can be one of the values in \ref MSA_Error
...
/*!	\brief Application-level code can call this method within their
	implementation of an \ref MSA_Callback_OnBrowse handler when they need to 
	...
	\param[in] cds_query CDS query specified in the \ref MSA_Callback_OnBrowse callback.
...
/*!	\brief Application-level code can call this method within their
	implementation of \ref MSA_Callback_OnBrowse when they need to respond to the
...
/*!	\brief Application-level code can call this method within their
	implementation of \ref MSA_Callback_OnBrowse when they need to finish the
