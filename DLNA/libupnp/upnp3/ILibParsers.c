...
void ILibStartChain(void *Chain)
{
	...
	while(((struct ILibBaseChain*)Chain)->TerminateFlag==0)
	{
		...
		if(slct==-1)
		{
			...
			FD_ZERO(&errorset);
			error_count++;
			if( error_count%256 == 0 )
			{
				printf("UPnP Select errno = %d\n", errno); 
				printf("This means %s\n", strerror(errno)); 
				error_count = 0;
			}
			usleep(100000);
...
#if defined(__SYMBIAN32__)
	return(0);
#elif defined(ENABLE_ANDROID_NDK) // FIXME
	return(0);
...
{
#if defined(ENABLE_ANDROID_NDK) // FIXME
        return(0);
#elif !defined(_WIN32_WCE) && !defined(__SYMBIAN32__)
	tzset();
	return(daylight);
#else
	//
